function dividirCadena(cadenaADividir,separador) {
    ...
    for (var i=0; i < arrayDeCadenas.length; i++) {
        arrayFinal.push(String(arrayDeCadenas[i]));
    }
    return arrayFinal;
}
(function () {
    ...
    callWithJQuery(function ($) {
        ...
        Utilities
         */
        var PivotData, addSeparators, aggregatorTemplates, aggregators, dayNamesEn, derivers, getSort, locales,
            mthNamesEn, naturalSort, numberFormat, pivotTableRenderer, rd, renderers, rx, rz, sortAs, usFmt, usFmtInt,
            usFmtPct, zeroPad;
        ...
        locales = {
            en: {
                ...
                localeStrings: {
                    ...
                    tooMany: "(Demasiados para enumerar)",
                    filterResults: "Valores de filtro",
                    apply: "Aplicar",
                    cancel: "Cancelar",
                    totals: "Total",
                    vs: "vs",
        ...
        pivotTableRenderer = function (pivotData, opts) {
            ...
            if (rowAttrs.length !== 0) {
                ...
                for (i in rowAttrs) {
                    ...
                    th1.className = "pvtAxisLabel";
                   
                    tr1.appendChild(th1);
                }
                th2 = document.createElement("th");
                if (colAttrs.length === 0) {
                    th2.className = "pvtTotalLabel";
                    th2.innerHTML = opts.localeStrings.totals;
                    ...
                    if(i%2==0)
                        {
                            ...
                            th3.className = "pvtAxisLabelele";
                            th3.textContent = "Suma";
                            tr1.appendChild(th3);
                        }
                        else{
                            th3 = document.createElement("th");
                            th3.className = "pvtAxisLabele";
                            th3.textContent = "Cant"
                            tr1.appendChild(th3);
                        }
            ...
            $(".pvtAxisLabele:last-child").remove();
            ...
            for (i in rowKeys) {
				...
				totalAggregator1 = pivotData.getAggregator(rowKey, []);
				val1 = totalAggregator1.value();
                ...
                for (j in colKeys) {
					...
					var multiplica=(val*100);
					var porcentaje=0;
					var flecha='';
                    ////agregar suma
                    var valid=val.toString().indexOf(",");
                    if(valid!=-1)
                    {
                        var sumaT=val.split(",");
                        var sumaTT=0;
                        sumaT.forEach( function(valor, indice, array) {
                            sumaTT=sumaTT+parseFloat(valor.trim());
                        });
                        porcentaje=sumaTT.toFixed(2);
                     //   val=sumaT.length;
                    ...
                    a.className = "pvtRowLabel";
                    a.setAttribute("data-hover", "tooltip");
                    a.setAttribute("data-placement", "top");
                    a.setAttribute("data-target", "#Modalagregar");
                    a.setAttribute("data-toggle", "modal");
                    a.setAttribute("id", "modal");
                  //  a.setAttribute("onclick", "EditChangesIntern("+arrayFinal+")");
                    a.textContent = val;
                    ...
                    td1.className = "pvtVal row" + i + " col" + j;
                    var a = document.createElement("a");
                    a.className = "pvtRowLabel";
                    a.setAttribute("data-hover", "tooltip");
                    a.setAttribute("data-placement", "top");
                    ...
                    a.setAttribute("id", "modal");
                  //  a.setAttribute("onclick", "EditChangesIntern("+arrayFinal+")");
                    a.textContent = porcentaje;
					
                    td1.appendChild(a);
                    // td.textContent = aggregator.format(val);
                    td1.setAttribute("data-value", val);
                    if (getClickHandler != null) {
                        td1.onclick = getClickHandler(val, rowKey, colKey);
                    }
                    tr.appendChild(td);
                    tr.appendChild(td1);
                ...
                if(valid!=-1)
                {
                    var sumaT3=val.split(",");
                    var sumaTT3=0;
                    sumaT3.forEach( function(valor, indice, array) {
                        sumaTT3=sumaTT3+parseFloat(valor.trim());
                    });
                    porcentaje3=sumaTT3.toFixed(2);
                   // val=sumaT3.length;
                ...
                a.setAttribute("data-toggle", "modal");
                a.setAttribute("id", "modal");
               // a.setAttribute("onclick", "EditChanges(\'" + contenido + "\',\'" + parametro + "\',\'" + columna + "\',\'" + parcol + "\')");
                a.textContent = val;
                td.appendChild(a);
                td.setAttribute("data-value", val);
                if (getClickHandler != null) {
                ...
                //2----
                td1 = document.createElement("td");
                td1.className = "pvtTotal rowTotal";
                ...
                a.setAttribute("id", "modal");
            //    a.setAttribute("onclick", "EditChanges(\'" + contenido + "\',\'" + parametro + "\',\'" + columna + "\',\'" + parcol + "\')");
                a.textContent = porcentaje3;
                td1.appendChild(a);
                td1.setAttribute("data-value", val);
                if (getClickHandler != null) {
                    td1.onclick = getClickHandler(val, rowKey, []);
                }
                td1.setAttribute("data-for", "row" + i);
            ...
            tr.appendChild(th);
			  for (j in colKeys) {
                if (!hasProp.call(colKeys, j)) continue;
                colKey = colKeys[j];
                totalAggregator3 = pivotData.getAggregator([], colKey);
                
            }
			totalAggregator3 = pivotData.getAggregator([], []);
            val3 = totalAggregator3.value();
            for (j in colKeys) {
                ...
                {
                        ////agregar suma
                        var sumaT2=val.split(",");
                        var sumaTT2=0;
                        sumaT2.forEach( function(valor, indice, array) {
                            sumaTT2=sumaTT2+parseFloat(valor);
                        });
                        var porcentaje2=sumaTT2.toFixed(2);
                        //fin suma
                ...
                td1 = document.createElement("td");
                td1.className = "pvtTotal colTotal";
                td1.textContent = porcentaje2;
                td1.setAttribute("data-value", val);
                if (getClickHandler != null) {
                    td1.onclick = getClickHandler(val, [], colKey);
                }
                td1.setAttribute("data-for", "col" + j);
            ...
            var porcentaje4=0;
                ...
                {
                        ////agregar suma
                        var sumaT4=val.split(",");
                        var sumaTT4=0;
                        sumaT4.forEach( function(valor, indice, array) {
                            sumaTT4=sumaTT4+parseFloat(valor);
                        });
                        var porcentaje4=sumaTT4.toFixed(2);
                        //fin suma
            ...
            td1= document.createElement("td");
            td1.className = "pvtGrandTotal";
            td1.textContent = porcentaje4;
            td1.setAttribute("data-value", val);
            if (getClickHandler != null) {
                td1.onclick = getClickHandler(val, [], []);
            }
        ...
        $.fn.pivotUI = function (input, inputOpts, overwrite, locale) {
            var a, aggregator, attr, attrLength, attrValues, colOrderArrow, defaults, e, existingOpts, fn1, i,
                initialRender, l, len1, len2, len3, localeDefaults, localeStrings, materializedInput, n, o, opts,
                ordering, pivotTable, recordsProcessed, ref, ref1, ref2, ref3, refresh, refreshDelayed, renderer,
                rendererControl, rowOrderArrow, shownAttributes, tr1, tr2, uiTable, unused,
            ...
            try {
                ...
                fn1 = function (attr) {
                    var attrElem, checkContainer, closeFilterBox, controls, filterItem, filterItemExcluded,
                        finalButtons, hasExcludedItem, len2, n, placeholder, ref1, sorter, triangleLink, v, value,
                        valueCount, valueList, values;
                    values = (function () {
                        var results;
                        results = [];
                ...
                refreshDelayed = (function (_this) {
                    return function () {
                        var exclusions, inclusions, len4, newDropdown, numInputsToProcess, pivotUIOptions, pvtVals,
                            ref4, ref5, subopts, t, u, unusedAttrsContainer, vals;
                        subopts = {
